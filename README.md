...
- 图片对象右侧缩略图
- 非图片对象显示类型图标
- 对象列表补充更清楚的元信息展示：文件类型、MIME、创建时间、精确字节大小
- 大文件自动切片上传
- 前端显示实时速度与 ETA
- 上传完成后的友好成功提示
- 多选批量删除，支持当前结果全选 / 清空、已选数量显示、部分失败反馈与当前列表局部更新
- 删除失败列表过长时支持折叠收纳，避免反馈区域一下子拉太长
...
- 恢复未完成 multipart 会话时，服务端会额外查询 OCI 远端已存在 parts，并与本地 session 元数据对账
- 如果本次远端对账暂时失败，会给出明确提示，并保守降级为“先按本地 session 状态继续恢复；最终合并前仍再次校验”
- 如果本地 session 与 OCI 远端 parts 不一致，会优先以远端已确认的 part 列表修正本地可恢复状态
...
这轮先做**小步增强**，重点是让网络波动时的大文件上传更稳，也更容易看出到底为什么失败。

当前前端 / 接口层会区分以下几类错误：
- `timeout`：请求超时，可重试
- `connection`：连接中断 / 提前断开，可重试
- `http_5xx`：OCI 或上游服务端异常，可重试
- `http_4xx`：明显客户端侧 / 请求被拒绝类错误，默认不重试
- `unknown`：暂时无法归类的异常，默认不重试

当前行为：
- 可重试错误会继续自动重试
- 不同错误类型使用不同等待时间
- 明显不可重试错误会尽早停止，避免空转
- 前端状态文案会显示“失败原因 + 是否继续重试 / 已停止重试”
- 服务端分片上传接口会返回更明确的 `error_code / retryable / reason`
...
- 从这一轮开始，恢复上传或查询上传状态时，会对 OCI 远端 multipart parts 做一次轻量对账，服务重启后也更容易跳过已经真正上传成功的分片
